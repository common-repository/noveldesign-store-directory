    ...
    if (!empty($_FILES['import_file']['name']) && $extension == 'csv')
    {
        ...
        while(($csvData = fgetcsv($csvFile)) !== false)
        {
            ...
            if (isset($wp_shop_category) && isset($post_id))
            {
                ...
                {
                    $cat_name = get_term_by('name', $category, 'categories');
                    ...
                    {
                        $wpdocs_cat = array('cat_name' => $category, 'taxonomy' => 'categories');
                ...
                {
                    wp_set_post_terms($post_id, $post_id_array, 'categories');
                }
            }
            if (isset($post_id))
            {
    ...
    <div class="row">
	    <div class="col-md-12 head">
	        ...
	        <div class="">				        	
                ...
                <form style="display: inline;" method='post' action='<?php echo esc_url($_SERVER['REQUEST_URI']); ?>' enctype='multipart/form-data' >
                    <input class="button button-primary button-large" type="submit" name="butexport" value="Export">
                </form>
	        </div>
	    </div>
	    <div class="col-md-12"  id="importFrm" style="display: none;margin-top:10px;">
	        ...
	        <form method='post' action='<?php echo esc_url($_SERVER['REQUEST_URI']); ?>' enctype='multipart/form-data'>
			  ...
			  <input type="submit" name="butimport" value="Import" class="button button-primary button-large" >
			</form></div>
	    </div>
	</div>
	<script>
