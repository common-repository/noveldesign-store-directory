<?php
/*
 * Template Name: Store Directory Search
...
<?php endif; ?>	
			...
			<?php 
				$page_url = get_permalink(get_page_by_path("shop-list"));
				if(isset($page_url)):
			?>
			    <div class="col-md-12">			
			    	<form method="get"  action="<?php echo esc_url($page_url);?>">
			    			<input name="search" class="search-input" type="text" placeholder="Search for a store name i.e Woolworths, Edgars etc." aria-label="Search">
						    <button type="submit" class="search-submit" value="Search" placeholder="To search, type and hit enter.">Search Stores</button>
						</form>
						</div>
			    <div class="col-md-12">	
			    ...
			    <?php
                    foreach (range('A', 'Z') as $char) {
    					...
    					$postids = $wpdb->get_col($wpdb->prepare("
    						...
    						AND 		$wpdb->posts.post_type = 'wp_shops'
                ...
                <?php if(get_option('WPM_CAROUSEL_STATUS')): ?>
                    <div class="col-md-12 shop_carousel">
                        <hr />
                        <h2><?php echo _e('OUR STORES'); ?></h2>
                        <?php
                            $images = ndsd_shop::getImagesForMainPage();
                        ?>
